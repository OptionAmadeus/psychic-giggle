...
export class PortfolioService extends ApiService {
  async getAssets(): Promise<Asset[]> {
    const response = await this.get<{ data: Asset[] }>(
      API_CONFIG.endpoints.portfolio.assets,
    );
    return response.data;
  }

  async getTransactions(): Promise<Transaction[]> {
    const response = await this.get<{ data: Transaction[] }>(
      API_CONFIG.endpoints.portfolio.transactions,
    );
    return response.data;
  }
