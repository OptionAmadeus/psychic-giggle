...
const EDGE_FUNCTION_URL = 'https://dvkpanntfxehgasngylg.supabase.co/functions/v1/exchange-token';
...
export async function exchangeToken(code: string, codeVerifier: string): Promise<TokenResponse> {
  try {
    ...
    if (!data.access_token || !data.refresh_token) {
      throw CoinbaseOAuthError.tokenExchangeFailed('Invalid token response format');
    }

    return data;
  } catch (error) {
    console.error('Token exchange request failed:', error);
    throw error instanceof CoinbaseOAuthError ? error : 
